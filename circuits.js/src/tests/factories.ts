...
export async function makePublicKernelInputsWithTweak(
...
): Promise<PublicKernelCircuitPrivateInputs> {
  ...
  publicKernelInputs.previousKernel.publicInputs.end.publicCallStack[MAX_PUBLIC_CALL_STACK_LENGTH_PER_TX - 1] =
    new CallRequest(
      await publicCall.callStackItem.getCompressed().hash(),
...
export async function makeContractClassPublic(seed = 0): Promise<ContractClassPublic> {
