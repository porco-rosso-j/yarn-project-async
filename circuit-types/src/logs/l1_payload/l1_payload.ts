...
export abstract class L1Payload {
   ...
   */
  protected async _encrypt<T extends EncryptedLogIncomingBody>(
    ...
    const outgoingBodyCiphertext = await new EncryptedLogOutgoingBody(ephSk, recipient, ivpkApp).computeCiphertext(
   ...
   */
  protected static async _decryptAsIncoming<T extends EncryptedLogIncomingBody>(
    data: Buffer,
    ivsk: GrumpkinScalar,
    fromCiphertext: (incomingBodySlice: Buffer, ivskApp: GrumpkinScalar, ephPk: Point) => Promise<T>,
  ): Promise<[AztecAddress, T]> {
   ...
   * @returns The decrypted log payload
   */
  protected static async _decryptAsOutgoing<T extends EncryptedLogIncomingBody>(
    data: Buffer,
    ovsk: GrumpkinScalar,
    fromCiphertext: (incomingBodySlice: Buffer, ivskApp: GrumpkinScalar, ephPk: Point) => Promise<T>,
  ): Promise<[AztecAddress, T]> {
    ...
    const outgoingBody = await EncryptedLogOutgoingBody.fromCiphertext(
      reader.readBytes(OUTGOING_BODY_SIZE),
      ovskApp,
      ephPk,
    );
  ...
  protected static async ensureMatchedMaskedContractAddress(
    contractAddress: AztecAddress,
    randomness: Fr,
