...
export class TaggedLog<Payload extends L1NotePayload | L1EventPayload> {
  ...
  public async encrypt(
    ...
    return serializeToBuffer(
      this.incomingTag,
      this.outgoingTag,
      await this.payload.encrypt(ephSk, recipient, ivpk, ovKeys),
    );
  }
  ...
  ): Promise<TaggedLog<L1EventPayload> | undefined>;
  static async decryptAsIncoming(
    data: Buffer | bigint[],
    ivsk: GrumpkinScalar,
    payloadType?: typeof L1NotePayload,
  ): Promise<TaggedLog<L1NotePayload> | undefined>;
  static async decryptAsIncoming(
    data: Buffer | bigint[] | EncryptedL2Log,
    ivsk: GrumpkinScalar,
    payloadType: typeof L1NotePayload | typeof L1EventPayload = L1NotePayload,
  ): Promise<TaggedLog<L1NotePayload | L1EventPayload> | undefined> {
    try {
      if (payloadType === L1EventPayload) {
        ...
        const payload = await L1EventPayload.decryptAsIncoming(data as EncryptedL2Log, ivsk);
        return new TaggedLog(payload, incomingTag, outgoingTag);
      } else {
  ...
  static async decryptAsOutgoing(
    ...
    try {
      if (payloadType === L1EventPayload) {
        ...
        const incomingTag = Fr.fromBuffer(reader);
        const outgoingTag = Fr.fromBuffer(reader);
        const payload = await L1EventPayload.decryptAsOutgoing(data as EncryptedL2Log, ovsk);
        return new TaggedLog(payload, incomingTag, outgoingTag);
      } else {
        const input = Buffer.isBuffer(data) ? data : Buffer.from((data as bigint[]).map((x: bigint) => Number(x)));
        const reader = BufferReader.asReader(input);
