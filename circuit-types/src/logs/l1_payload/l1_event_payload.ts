...
export class L1EventPayload extends L1Payload {
  ...
  public async encrypt(ephSk: GrumpkinScalar, recipient: AztecAddress, ivpk: PublicKey, ovKeys: KeyValidationRequest) {
    return await super._encrypt(
   ...
   */
  public static async decryptAsIncoming(encryptedLog: EncryptedL2Log, ivsk: GrumpkinScalar) {
    ...
    const [address, incomingBody] = await super._decryptAsIncoming(
      reader.readToEnd(),
      ivsk,
   ...
   * @returns The decrypted log payload
   */
  public static async decryptAsOutgoing(encryptedLog: EncryptedL2Log, ovsk: GrumpkinScalar) {
    const reader = BufferReader.asReader(encryptedLog.data);
    ...
    const [address, incomingBody] = await super._decryptAsOutgoing(
      reader.readToEnd(),
      ovsk,
      EncryptedEventLogIncomingBody.fromCiphertext,
    );
